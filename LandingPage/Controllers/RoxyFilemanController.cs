...
namespace CTH.QuizSystem.Web.Controllers
{
    ...
    public class RoxyFilemanController : Controller
    {
        ...
        {
            _settings = JsonConvert.DeserializeObject<Dictionary<string, string>>(System.IO.File.ReadAllText(_systemRootPath + "/wwwroot/plugins/fileman/conf.json"));
            string langFile = _systemRootPath + "/wwwroot/plugins/fileman/lang/" + GetSetting("LANG") + ".json";
            if (!System.IO.File.Exists(langFile)) langFile = _systemRootPath + "/wwwroot/plugins/fileman/lang/en.json";
        ...
        [HttpPost]
        public IActionResult UPLOAD(string d)
        {
            try
            {
                d = MakePhysicalPath(d);
                ...
                else return Content("<script>parent.fileUploaded(" + res + ");</script>");
            }
            catch (Exception ex)
            {
                if (!IsAjaxUpload()) return Content("<script>parent.fileUploaded(" + GetErrorRes(LangRes("E_UploadNoFiles")) + ");</script>");
                else return Content(GetErrorRes(ex.Message));
            }
        }
        ...
        */
        ...
        private string GetFileType(string ext)
        {
            ...
            var strMediaType = GetSetting("MEDIATYPE");
            var mediaType = new List<string>();
            if (!string.IsNullOrEmpty(strMediaType))
            {
                mediaType.AddRange(Regex.Split(strMediaType, "\\s+"));
            }
            ...
            else if (mediaType.Contains(ext)) ret = "media";
            return ret;
        }
